	...
	for {
		...
		case "user.message":
			...
			if err := s.store.StartSession(sess.ID, prompt); err != nil {
				log.Printf("server: failed to start session store: %v", err)
			}
			if err := s.tokenStore.StartSession(sess.ID); err != nil {
				log.Printf("server: failed to start token store session: %v", err)
			}
			...
			s.bus.Subscribe(func(ev events.Event) {
				...
				if appendErr := s.store.AppendEvent(sess.ID, ev); appendErr != nil {
					log.Printf("server: failed to append event: %v", appendErr)
				}
				// Finish session on terminal events
				if ev.Type == events.EventTypeSessionFinished {
					if finishErr := s.store.FinishSession(sess.ID, "done"); finishErr != nil {
						log.Printf("server: failed to finish session: %v", finishErr)
					}
			...
			// Build the SubAgentRunner factory (closes over server deps).
				...
				runAgent = func(ctx context.Context, def *registry.AgentDefinition, sessionID string, message string) (string, error) {
					...
					subToolReg := tools.NewRegistry(s.sb, nil, s.bus, s.reg, runAgent, sessionID)
				...
				toolReg := tools.NewRegistry(s.sb, nil, s.bus, s.reg, runAgent, sess.ID)
