...
// buildTeamContext generates the ## Team section for leader agents.
// If a child has no description, the child's Name is used as fallback.
func buildTeamContext(children []*registry.AgentNode) string {
	if len(children) == 0 {
		return ""
	}
	var b strings.Builder
...
func (r *Runner) Run(ctx context.Context, userMessage string) (string, error) {
	...
	consecutiveErrors := 0
	consecutiveNudges := 0
	const maxConsecutiveNudges = 5
	totalToolCalls := 0

	// Build team context once per Run() — it only changes if the tree changes,
	// which does not happen mid-session.
	teamCtx := ""
	if r.agent.Definition.Type == TypeLeader {
		teamCtx = buildTeamContext(r.agent.Definition.Children)
	}
	...
	for {
		...
		if len(resp.ToolCalls) == 0 {
			...
			if consecutiveNudges >= maxConsecutiveNudges {
				return "", fmt.Errorf("runner: agent %q returned %d consecutive responses without tool calls; session terminated", agentName, consecutiveNudges)
			}
			r.chat.AddText(llm.RoleUser, "Continue. Use a tool to proceed or call finish_work if you are done.")
			continue
		}

		// Reset nudge counter when tool calls are received.
		consecutiveNudges = 0
		...
		for _, call := range resp.ToolCalls {
			// If finish_work was already called in this batch, add a skipped
			// result for any remaining calls to satisfy the Anthropic protocol.
			if finished {
				toolResults = append(toolResults, llm.ToolResult{
					ToolUseID: call.ID,
					Content:   "skipped: finish_work already called",
					IsError:   false,
				})
				continue
			}
			...
			if errors.Is(execErr, tools.ErrFinishWork) {
				// Clean stop — record the result and continue to collect
				// tool results for any remaining calls in this batch.
				finishResult = result
				finished = true
		...
		// Append ALL tool results as a user message before any early return,
		// so the message history is always consistent with the Anthropic protocol.
		r.chat.AddMessage(llm.NewToolResultMessage(toolResults))

		// Enforce retry limit after the full batch has been processed.
		if retryLimitExceeded {
			return "", fmt.Errorf("runner: tool %q failed %d consecutive times, last error: %w", lastFailedTool, consecutiveErrors, lastExecErr)
		}
